...
for /d %%f in (addons\*) do (
    ...
    !armake! build -i include -w unquoted-string -w excessive-concatenation -f !folder! @tac_mods\addons\tac_!name!.pbo
)
...
for /d %%f in (optionals\*) do (
    ...
    !armake! build -i include -w unquoted-string -w excessive-concatenation -f !folder! @tac_mods\optionals\tac_!name!.pbo
)
