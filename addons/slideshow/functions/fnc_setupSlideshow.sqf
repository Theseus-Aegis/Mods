...
private ["_objects", "_controllers", "_images", "_names", "_controller"];
...
TRACE_2("Information before Synced objects",_objects,_controllers);

// Objects synced to the module
{
    // Objects + Both (reference CfgVehicles ModuleDescription)
    if (typeOf _x in ["Land_MapBoard_F", "Land_Laptop_unfolded_F", "Land_Laptop_device_F", "Land_PCSet_01_screen_F"]) then {
        _objects pushBack _x;
    };
    // Both + Controllers (reference CfgVehicles ModuleDescription)
